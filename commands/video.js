...
async function videoCommand(sock, chatId, message) {
    try {
        ...
        if (!searchQuery) {
            await sock.sendMessage(chatId, { text: 'මොකක්ද ඔයාට download කරල ඕන?' }, { quoted: message });
            return;
        }
        ...
        if (searchQuery.startsWith('http://') || searchQuery.startsWith('https://')) {
            ...
            if (!videos || videos.length === 0) {
                await sock.sendMessage(chatId, { text: 'මට සොයාගත නොහැකිවිය!' }, { quoted: message });
                return;
            }
        ...
        try {
            ...
            if (meta?.success && meta?.result?.download_url) {
                ...
                await sock.sendMessage(chatId, { text: 'api කේතයේ දෝෂයකි. පසුව උත්සහ කරන්න.' }, { quoted: message });
                return;
            }
        } catch (e) {
        ...
        if (download403) {
            ...
            try {
                ...
                await sock.sendMessage(chatId, { text: 'cdn බාගැනිම අසාර්ථකයි.' }, { quoted: message });
                return;
            }
        }
        if (!buffer || buffer.length < 1024) {
            await sock.sendMessage(chatId, { text: 'කිසිවක් නැත (empty).' }, { quoted: message });
            return;
        }
        ...
        try {
            ...
            if (!fs.existsSync(convertedFile)) {
                await sock.sendMessage(chatId, { text: 'අතුරුදහන්.' }, { quoted: message });
                return;
            }
            ...
            try {
                if (!fs.existsSync(tempFile)) {
                    await sock.sendMessage(chatId, { text: 'උෂ්ණත්ව ෆෝල්ඩරය අතුරුදහන්.' }, { quoted: message });
                    return;
                }
                const origStats = fs.statSync(tempFile);
                const maxSize = 62 * 1024 * 1024; // 62MB
                if (origStats.size > maxSize) {
                    await sock.sendMessage(chatId, { text: 'විෂාල වීඩියෝවකි.' }, { quoted: message });
                    return;
                }
            } catch {}
        ...
        await sock.sendMessage(chatId, { text: 'අසාර්ථකයි: ' + (error?.message || 'හඳුනා ගැනීම අසාර්ථකයි') }, { quoted: message });
    }
}
